...
export default function UserCard({ user }: { user: UserProfile }) {
    ...
    return (
        <div className="relative">
            <Button variant="tertiary" isActive={isOpen} className="justify-center w-full items-center gap-2" onClick={() => {setOpen(true)}}>{user.picture && <img src={user.picture} alt={user.name ?? ''} className="w-8 h-8 rounded-full" />} <span className="max-md:hidden">{user.name}</span></Button>
