    ...
    return (
        <div className="py-2 grid col-start-1 col-span-6 sm:col-span-8 grid-cols-subgrid items-center overflow-auto">
            ...
            <Span className="capitalize  whitespace-nowrap w-full flex justify-between items-center gap-2"><span className={'max-md:hidden'}>{status}</span><button disabled={loading} className="hover:scale-95 active:scale-95" onClick={async (event) => {
                ...
                setLoading(false);
            }} title={status === 'completed' ? 'Mark as In Progress' : 'Mark as Completed'}>{status === 'completed' ? <RemoveDone /> : <Check />}</button><button disabled={loading} className="hover:scale-95 active:scale-95" onClick={async (event) => {
                event.preventDefault();
                event.stopPropagation();
                if(!task._id) {
                    return;
            ...
            {!compact && <div className="col-start-2 col-span-7 flex gap-2 py-2 overflow-hidden" >{task.tags?.slice(0, 5).map((tag, index) => <Tag key={index} hideClose tag={tag}/>)}</div>}
        </div>
    )
}