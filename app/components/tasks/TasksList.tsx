...
export default function TaskList({ tasks, compact } : { tasks: Task[], compact?: boolean }) {
    return (
        ...
        //     ))}
        // </Box>
        <Box className={`grid grid-cols-8 ${compact ? 'gap-y-4' : 'gap-y-8'} w-full p-2 py-4 md:p-8 md:rounded-md`}>
            <span className="mb-4 md:text-lg col-span-2 md:col-span-1">Due</span>
            <span className="mb-4 md:text-lg col-span-3 md:col-span-2">Title</span>
            <span className="mb-4 md:text-lg hidden sm:inline sm:col-span-2 md:col-span-4">Description</span>
            <span className="mb-4 md:text-lg hidden sm-inline overflow-x-hidden text-ellipsis">Status</span>
            { tasks.map((task, index) => <TaskCard key={index} task={task} compact={compact}/>) }
        </Box>
    )
}